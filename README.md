...
1. Зарегистрируйтесь на Render (проще всего — через GitHub, без подтверждения по почте).
2. В дашборде: **New → Blueprint**, укажите репозиторий `tripmate` — Render прочитает `render.yaml` и создаст сервис
   автоматически. Либо **New → Web Service** и вручную указать Build Command `bash build.sh`,
   Start Command `gunicorn tripmate_project.wsgi:application`.
3. После первого деплоя создайте суперпользователя через вкладку **Shell** сервиса в дашборде Render:
   `python manage.py createsuperuser`. Там же можно выполнить `python manage.py seed_data` для тестовых данных.

**Важный нюанс:** на бесплатном тарифе Render диск эфемерный (не сохраняется между пересборками сервиса) — если не
подключить отдельную Postgres, база SQLite и загруженные фото будут сбрасываться при каждом новом деплое. Для
демонстрации/защиты проекта этого достаточно; для постоянной эксплуатации стоит подключить бесплатную Postgres от
Render и хранение медиафайлов во внешнем сторадже (например, S3-совместимом).
