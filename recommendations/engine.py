...
Это не нейросеть и не ML-модель, а прозрачная скоринговая (rule-based)
система: каждый показатель — число от 0 до 1, вес показывает, насколько он
важен относительно других. Такой подход достаточен для MVP и легко
объясним пользователю ("почему мне это порекомендовали").
"""
